...
datos <- readxl::read_excel("C:/NAS_PAU/TFG/BBDD_Femenino/DatasetCompletoFemenino.xlsx")
...
# Majors (binarias)
datos$Chevron <- factor(datos$Chevron, levels = c(0,1), labels = c("No", "Sí"))
datos$US_Open <- factor(datos$US_Open, levels = c(0,1), labels = c("No", "Sí"))
datos$KPMG <- factor(datos$KPMG, levels = c(0,1), labels = c("No", "Sí"))
datos$Amundi <- factor(datos$Amundi, levels = c(0,1), labels = c("No", "Sí"))
datos$AIG <- factor(datos$AIG, levels = c(0,1), labels = c("No", "Sí"))
...
vars_categoricas <- c(
  ...
  "campo_golf", "indoor",
  "Chevron", "US_Open", "KPMG", "Amundi", "AIG",
  "es_usa", "emocion", "dia_semana",
  "fin_de_semana", "juegos_olimpicos"
)
...
for (var in vars_categoricas) {
  boxplot(n_likes ~ get(var), data = datos.comentarios, main = paste("Likes por", var), xlab = var, outline = FALSE)
}
cols_top <- grep("_top$", names(datos), value = TRUE)
for (top in cols_top) {
  boxplot(n_likes ~ get(top), data = datos.comentarios, main = paste("Likes por", top), xlab = top, outline = FALSE)
}
par(mfrow = c(1, 1))
```
```{r}
...
vars_categoricas <- c(
  ...
  "campo_golf", "indoor",
  "Chevron", "US_Open", "KPMG", "Amundi", "AIG",
  "es_usa", "emocion", "dia_semana", "juegos_olimpicos"
)
...
res_likes <- export_many_pdfs(
  file_stub = "likes_categoricasFEM_3vertical",
...
res_coment <- export_many_pdfs(
  file_stub = "comentarios_categoricasFEM_3vertical",
...
if (!is.null(res_likes$summary)) {
  write.csv(res_likes$summary,
            file = file.path(out_dir, "modelo3_boxplots_resumen.csv"),
...
latex_likes <- make_latex_block(
  res_likes$pdfs, "Diagramas de caja del Modelo 3", "boxlikes"
)
latex_coment <- make_latex_block(
  res_coment$pdfs, "Diagramas de caja del Modelo 4", "boxcoment"
)

writeLines(c("% ----- BLOQUE AUTO: Boxplots categóricos (3 variables/hoja) -----",
             latex_likes, latex_coment),
           con = file.path(out_dir, "bloque_boxplots_categoricosFEM_3porhoja.tex"))
...
vars_continuas <- c('posicion_rr', 'n_fotos', "n_videos","n_palabras","n_hashtags", "n_menciones", "n_emojis","n_rostros", "edad", "inicio_lpga",'dinero_2024', 'n_otros_circuitos', 'n_torneos_total', 'n_majors', 'seguidores', 'publicaciones',
                    'n_likes', 'n_comentarios')
...
vars_continuas_estudio <- c('posicion_rr', 'n_fotos', "n_videos", "n_palabras","n_hashtags", "n_menciones", 
                            "n_emojis","n_rostros", "edad", "inicio_lpga",'dinero_2024', 'n_torneos_total',
                            'n_torneos_otros_circuitos', 'seguidores', 'publicaciones',
                            'n_likes', 'n_comentarios')
...
```{r}
# =========================================================
# Densidades Bruto/Raíz/Log por variable (FEM)
# -> 3 variables por hoja (rejilla 3x3)
# -> Un PDF por página (p01, p02, ...)
...
vars_continuas_estudio <- c(
  'posicion_rr','n_fotos','n_videos','n_palabras','n_hashtags','n_menciones',
  'n_emojis','n_rostros','edad','inicio_lpga','dinero_2024','n_torneos_total',
  'n_torneos_otros_circuitos','seguidores','publicaciones','n_likes','n_comentarios'
)
...
export_model_batches <- function(datos_modelo, modelo_tag = c("modelo1","modelo2"),
  ...
  for (i in seq_along(chunks)) {
    ...
    file_pdf <- file.path(out_dir, sprintf("%s_densidadesFEM_p%02d.pdf", modelo_tag, i))
...
# ---------------- Ejecutar ----------------
# Modelo 3 = likes, Modelo 4 = comentarios (FEM)
...
latex_modelo1 <- make_latex_block(res1$pdfs, "Distribuciones del Modelo 3", "modelo1")
latex_modelo2 <- make_latex_block(res2$pdfs, "Distribuciones del Modelo 4", "modelo2")
writeLines(c("% ----- BLOQUE AUTO: Densidades (3 variables/hoja) -----",
             latex_modelo1, latex_modelo2),
           con = file.path(out_dir, "bloque_densidades_3porhojaFEM.tex"))
message("LaTeX escrito en: ",
        normalizePath(file.path(out_dir, "bloque_densidades_3porhojaFEM.tex"), winslash = "/"))
...
modelo.likes <- lmer(
  n_likes.log ~ 
    ...
    Chevron + US_Open + KPMG + Amundi + AIG +
    es_usa + emocion + dia_semana + juegos_olimpicos +
    nellykorda_top + ruoningyin1_top + lydsko_top +
    jeeno.atthaya_top +	liliavu_top +	hannahgreengolf_top +
    haeran_ryu_top + ayaka_furue27_top + celineboutier_top + charley.hull_top +
    (1 | usuario),
  data = datos.likes,
  REML=FALSE
)
...
saveRDS(bootstrap_resultados_likes, "C:/NAS_PAU/TFG/AnalisisGolf/bootstrap_5000_likes_LPGA.rds")
#bootstrap_resultados_likes <- readRDS("C:/NAS_PAU/TFG/AnalisisGolf/bootstrap_5000_likes_LPGA.rds")
...
saveRDS(bootstrap_resultados_comentarios, "C:/NAS_PAU/TFG/AnalisisGolf/bootstrap_5000_comentarios_LPGA.rds")
#bootstrap_resultados_comentarios <- readRDS("C:/NAS_PAU/TFG/AnalisisGolf/bootstrap_5000_comentarios_LPGA.rds")
...
nombres_bonitos <- c(
  ...
  "posicion_rr.log" = "Ranking mundial (Trf Log)",
  "inicio_lpga" = "Inicio LPGA",
  ...
  "ChevronSí" = "Chevron Championship",
  "US_OpenSí" = "US Women's Championship",
  "KPMGSí" = "KPMG Women's PGA Championship",
  "AmundiSí" = "Amundi Evian Championship",
  "AIGSí" = "AIG Women's Open",
  "carruselSí" = "Carrusel",
  "n_fotos" = "Nº Fotos",
  "n_videos" = "Nº Videos",
  ...
  "nellykorda_topSí" = "Nelly Korda",
  "ruoningyin1_topSí" = "Ruoning Yin",
  "lydsko_topSí" = "Lydia Ko",
  "jeeno.atthaya_topSí" = "Jeeno Thitikul",
  "liliavu_topSí" = "Lilia Vu",
  "hannahgreengolf_topSí" = "Hannah Green",
  "haeran_ryu_topSí" = "Haeran Ryu",
  "ayaka_furue27_topSí" = "Ayaka Furue",
  "celineboutier_topSí" = "Celine Boutier",
  "charley.hull_topSí" = "Charley Hull",
...
bloques <- list(
  ...
  "Información deportiva" = c("Ranking mundial (Trf Log)", "Inicio LPGA", "Dinero 2024 (Trf Log)", "Torneos jugados", "Otros circuitos", "Torneos Majors"),
  "Publicaciones en Majors y JJOO" = c("Juegos Olímpicos", "Chevron Championship", "US Women's Championship", "KPMG Women's PGA Championship", "Amundi Evian Championship", "AIG Women's Open"),
  ...
  "Jugadoras destacadas" = c("Nelly Korda", "Ruoning Yin","Lydia Ko", "Jeeno Thitikul","Lilia Vu","Hannah Green","Haeran Ryu",
  "Ayaka Furue","Celine Boutier","Charley Hull")
)
