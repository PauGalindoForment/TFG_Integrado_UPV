...
df_torneos["jugador_normalizado"] = df_torneos["Jugador"].apply(normalizar)

# Juegos Olimpicos: Tour == OGC 
olimpicos = df_torneos[df_torneos["Tour"] == "OGC"] \
    .drop_duplicates(subset=["jugador_normalizado"]) \
    .assign(Juegos_Olimpicos=1)[["jugador_normalizado", "Juegos_Olimpicos"]]

# Nº Otros Torneos: excluir PGAT, OGC, Maj y los que empiezan por "Previous"
otros_torneos = df_torneos[
    ~df_torneos["Tour"].isin(["PGAT", "OGC", "Maj"]) &
    ~df_torneos["Tour"].str.startswith("Previous", na=False)
].groupby("jugador_normalizado").size().reset_index(name="Nº Otros Torneos")

# Nº PGAT
pgat_torneos = df_torneos[df_torneos["Tour"] == "PGAT"] \
